...
auto cacheGame() -> void
{
	while (true)
	{
		...
		for (int i = 1; i < 64; i++)
		{
			...
			if (localTeam == playerTeam) //team check 
				//if (localTeam != playerTeam) //delete "//" for open deathmatch mode on here
				continue;
...
void espLoop()
{
	...
	if (global_pawn) {
		...
		{
			ImColor Red = { 250, 92, 255, 255 };
			std::string Watermark = "expectional "; //cancelled feature
		...
		for (CS2Entity CachePlayers : PlayerList)
		{
			...
			if (screenpos.z >= 0.01f)
			{
				...
				if (Settings::Visuals::bHealth)
				{
					...
					std::string Out69 = "" + std::to_string(static_cast<int32_t>(CachePlayers.health)) + "";
